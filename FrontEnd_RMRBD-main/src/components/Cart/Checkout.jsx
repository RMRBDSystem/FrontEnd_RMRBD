...
const Checkout = () => {
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      if (!selectedAddress.accountName && !selectedAddress.userName) {
        toast.error('Thông tin người nhận chưa được tải');
        return;
      }
      ...
      const newOrderId = bookOrderResponse.data.orderId;

      // Update each BookOrderDetail to point to the new order and then delete old orders
      for (const order of selectedOrders) {
        try {
          // Step 1: Update the BookOrderDetail to point to the new order
          await axios.put(
            `https://rmrbdapi.somee.com/odata/BookOrderdetail/${order.orderId}/${order.bookId}`,
            {
              ...order,
              orderId: newOrderId
            },
          ...
          // Step 2: Delete the old order if this was its last detail
          const orderToCheck = order.orderId;
          if (orderToCheck) {
            await axios.delete(
              `https://rmrbdapi.somee.com/odata/BookOrder/${orderToCheck}`,
              {
                headers: {
                  'Content-Type': 'application/json',
                  'Token': '123-abc'
            ...
            console.log(`Old order ${orderToCheck} deleted successfully.`);
          }
        } catch (error) {
          console.error('Error updating/deleting order:', error);
      ...
      await Swal.fire({
        position: "center", // Changed to center for better visibility
        icon: "success",
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm">
                    <span className="text-gray-500">Số Dư: {formatCurrency(userCoins)}</span>
                    {paymentMethod === 'COINS' && userCoins < calculateCombinedTotalPrice() && (
                      <span className="text-red-500 ml-2">
                        (Cần: {formatCurrency(calculateCombinedTotalPrice())})
                      </span>
                    )}
                  </p>
                </div>
