...
export const saveRecipeRate = async (recipeId,accountId, ratepoint) => {
    const urlRecipeRate = "https://rmrbdapi.somee.com/RecipeRate";
...
export const getRecipeRatePoint = async (recipeId) => {
    try {
        ...
        const response = await axios.get(`https://rmrbdapi.somee.com/RecipeRate?$apply=filter(RecipeID eq ${recipeId})/aggregate(RATEPOINT with average as AvgRatePoint)`, {
            headers: {
                token: '123-abc'
            }
        });
