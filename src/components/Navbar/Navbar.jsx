...
const Navbar = () => {
  ...
  const handleCartClick = async () => {
    ...
    // Use LoadingPage only when navigating TO cart page
        ...
        >
          ...
          <ul className="nav-links-container">
            ...
            <Link 
              ...
              className="relative inline-flex items-center p-2 transition-transform duration-500"
            >
              <FaShoppingCart 
                className={`w-6 h-6 text-white hover:text-green-500 transition-colors duration-300 
                ${isCartAnimating ? 'animate-[wiggle_0.5s_ease-in-out]' : ''}`}
              />
              {cartItemCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-red-500 text-white 
                  w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold
                  animate-[popIn_0.3s_cubic-bezier(0.68,-0.55,0.265,1.55)]">
                  {cartItemCount}
                </span>
              )}
            </Link>
